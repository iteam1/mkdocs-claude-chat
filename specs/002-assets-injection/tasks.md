...
- [ ] T001 Verify `src/mkdocs_claude_chat/_internal/assets.py` exists as a stub
- [ ] T002 Verify `src/mkdocs_claude_chat/assets/chat.css` and `chat.js` exist as source files
...
- [ ] T003 [US1] Define `ASSETS_DIR = Path(__file__).parent.parent / "assets"` constant in `src/mkdocs_claude_chat/_internal/assets.py`
- [ ] T004 [US1] Implement `copy_to_site(site_dir: str) -> None` that creates `site_dir/assets/` and copies `chat.css` and `chat.js` in `src/mkdocs_claude_chat/_internal/assets.py`
- [ ] T005 [US1] Add `from __future__ import annotations`, type hints, and Google-style docstrings in `src/mkdocs_claude_chat/_internal/assets.py`
...
- [ ] T006 [US2] Implement `register(extra_css: list, extra_javascript: list) -> None` that appends `"assets/chat.css"` and `"assets/chat.js"` in `src/mkdocs_claude_chat/_internal/assets.py`
...
- [ ] T007 [P] Write unit tests for `register` and `copy_to_site` in `tests/test_assets.py` covering: CSS appended, JS appended, no duplicates on double call, files created, dir auto-created, files non-empty
